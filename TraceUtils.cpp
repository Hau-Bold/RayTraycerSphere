...
Point3f TraceUtils::traceCones(const Ray ray, const std::vector<Cone>& cones, const int &depth)
{
	float tnear = Constants::INFINIT;//to find nearest sphere
	Cone* nearestCone = nullptr;
	// find intersection of this ray with the sphere in the scene
	for (Cone cone : cones) {

		float t0 = Constants::INFINIT, t1 = Constants::INFINIT;
		if (cone.isRayIntersecting(ray, t0, t1)) {
			if (t0 < 0) t0 = t1;
			if (t0 < tnear) {
				tnear = t0;
				nearestCone = &cone;
	...
	Point3f surfaceColor = 0; // color of the ray/surfaceof the object intersected by the ray
	Point3f pointOfIntersection = ray.getOrigin() + ray.getDirection() * tnear; // point of intersection
	Point3f normalAtIntersection = nearestCone->getNormalAt(pointOfIntersection); // normal at the intersection point
	normalAtIntersection.normalize(); // normalize normal direction
									  // If the normal and the view direction are not opposite to each other
									  // reverse the normal direction. That also means we are inside the sphere so set
									  // the inside bool to true. Finally reverse the sign of IdotN which we want
	...
	if ((nearestCone->getTransparency() > 0 || nearestCone->getReflection() > 0) && depth < Constants::MAX_RAY_DEPTH) {
		//float facingratio = -ray.getDirection().dot(nhit);
		// change the mix value to tweak the effect
		float fresneleffect = getFresnelEffect(ray, normalAtIntersection, 0.1);
		// compute reflection direction (not need to normalize because all vectors
		// are already normalized)
		Point3f directionOfReflection = getDirectionOfReflection(ray, normalAtIntersection);
		Point3f reflection = traceCones(Ray(pointOfIntersection + normalAtIntersection * bias, directionOfReflection), cones, depth + 1);

		Point3f refraction = 0;
		// if the sphere is also transparent compute refraction ray (transmission)
		if (nearestCone->getTransparency()) {
			float ior = 1.1, eta = (inside) ? ior : 1 / ior; // are we inside or outside the surface?
			float cosi = -normalAtIntersection.dot(ray.getDirection());
			float k = 1 - eta * eta * (1 - cosi * cosi);
			Point3f refrdir = ray.getDirection() * eta + normalAtIntersection * (eta * cosi - sqrt(k));
			refrdir.normalize();
			refraction = traceCones(Ray(pointOfIntersection - normalAtIntersection * bias, refrdir), cones, depth + 1);
		}
		// the result is a mix of reflection and refraction (if the sphere is transparent)
		surfaceColor = (
			reflection * fresneleffect +
			refraction * (1 - fresneleffect) * nearestCone->getTransparency()) * nearestCone->getSurfaceColor();
	}
	else {
		//it's a diffuse object, no need to raytrace any further
		for (unsigned i = 0; i < cones.size(); ++i) {
			if (cones[i].getEmissionColor().m_X > 0) {
				// this is a light
				Point3f transmission = 1;
				Point3f lightDirection = cones[i].getCenter() - pointOfIntersection;
				lightDirection.normalize();
				for (unsigned j = 0; j < cones.size(); ++j) {
					if (i != j) {
						float t0, t1;
						if (cones[j].isRayIntersecting(Ray(pointOfIntersection + normalAtIntersection * bias, lightDirection), t0, t1)) {
							transmission = 0;
							break;
						}
					}
				}
				surfaceColor += nearestCone->getSurfaceColor() * transmission *
					std::max(float(0), normalAtIntersection.dot(lightDirection)) * cones[i].getEmissionColor();
			}
		}
	}
	return surfaceColor;// +sphere->emissionColor;
...
Point3f * TraceUtils::render(const std::vector<Cone> &cones, const unsigned int & width, const unsigned int & height)
{
	Point3f* image = new Point3f[width * height];
	Point3f*pixel = image;
	float invWidth = 1 / float(width), invHeight = 1 / float(height);
	...
	for (unsigned y = 0; y < height; ++y) {
		for (unsigned x = 0; x < width; ++x, ++pixel) {
			...
			Point3f raydir(x_Dir, y_Dir, -1);
			raydir.normalize();
			*pixel = traceCones(Ray(Point3f(0), raydir), cones, 0);
		}
	}
...
void TraceUtils::paintImage(const Point3f * image, const unsigned int & width, const unsigned int & height)
{
	...
	std::ofstream ofs("./cones.ppm", std::ios::out | std::ios::binary);
	ofs << "P6\n" << width << " " << height << "\n255\n";
